...
public class LessonYearViewSkin extends DateControlSkin<LessonYearView> {

    public LessonYearViewSkin(LessonYearView view) {
        super(view);
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 4; col++) {
                ...
                LessonYearMonthView yearMonthView = view.getMonthView(month);
                yearMonthView.setShowMonthArrows(false);
                yearMonthView.setShowTodayButton(false);
                yearMonthView.setShowUsageColors(true);
                yearMonthView.setClickBehaviour(LessonYearMonthView.ClickBehaviour.SHOW_DETAILS);
    ...
    private void updateMonths() {
        LessonYearView yearPage = getSkinnable();
        LocalDate date = yearPage.getDate();
        int year = date.getYear();

        for (Month month : Month.values()) {
            YearMonth yearMonth = YearMonth.of(year, month);
            LessonYearMonthView view = yearPage.getMonthView(month);
            view.setMinSize(0, 0);
