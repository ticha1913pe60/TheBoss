...
import com.petarzlatev.languageclasses.view.LessonMonthSheetView.ClickBehaviour;
 ...
 * and a view of type {@link YearView}. Alternatively the page can also display
 * months in columns via the {@link MonthSheetView} control. The application can
 * switch between these two views by calling
 * {@link #setDisplayMode(DisplayMode)}.
 *
 * <h3>YearView</h3>
 ...
 */
public class LessonYearPage extends LessonPageBase {

	private LessonYearView yearView;
	private LessonMonthSheetView monthSheetView;
	private ToggleButton displayModeButton;

	/**
	 * Constructs a new year page.
	 */
	public LessonYearPage() {
		getStyleClass().add("year-page"); //$NON-NLS-1$

		this.yearView = new LessonYearView();

		this.monthSheetView = new LessonMonthSheetView();
		this.monthSheetView
				.setCellFactory(param -> new LessonMonthSheetView.DetailedDateCell(param.getView(), param.getDate()));
		...
		setDateTimeFormatter(DateTimeFormatter.ofPattern(Messages.getString("YearPage.DATE_FORMAT"), Locale.getDefault())); //$NON-NLS-1$
		...
		displayModeProperty()
				.addListener(it -> displayModeButton.setSelected(getDisplayMode().equals(DisplayMode.COLUMNS)));
	...
	protected Skin<?> createDefaultSkin() {
		return new LessonYearPageSkin(this);
	}
	...
	private void updateDisplayModeIcon() {
		...
		graphic.getStyleClass().addAll("lesson-button-icon", "display-mode-icon");
		displayModeButton.setGraphic(graphic);
	}

	/**
	 * An enum used for setting the display mode of the {@link YearPage}. The page
	 * can display the 12 months of a year in a grid or a column layout.
	 *
	...
	private final ObjectProperty<DisplayMode> displayMode = new SimpleObjectProperty<>(this, "displayMode",
			DisplayMode.GRID);

	/**
	 * A property used to control whether the page should display the year in a grid
	 * format (with each cell containing a single month) or in a column format (each
	 * column representing one month).
	 *
	...
	/**
	 * Returns the {@link MonthSheetView} used by the page to display months in
	 * columns.
	 *
	 * @return the month sheet view
	 */
	public final LessonMonthSheetView getMonthSheetView() {
		return monthSheetView;
	}

	/**
	 * Returns the {@link YearView} used by the page to display months in a 4x3 grid
	 * layout.
	 *
	 * @return the year view
	 */
	public final LessonYearView getYearView() {
		return yearView;
	}
