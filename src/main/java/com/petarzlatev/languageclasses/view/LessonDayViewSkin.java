...
public class LessonDayViewSkin<T extends LessonDayView> extends DayViewBaseSkin<T> implements LoadDataSettingsProvider {
    ...
    private LessonDayEntryView draggedEntryView;
    ...
    public LessonDayViewSkin(T view) {
        super(view);
    ...
    private void addOrRemoveDraggedEntryView() {
        LessonDayView view = getSkinnable();
    ...
    private void layoutStandard(LessonDayView dayView, double contentX, double contentY, double contentWidth, double contentHeight) {
        List<LessonDayEntryView> entryViews = getChildren().stream().filter(node -> node instanceof LessonDayEntryView).map(node -> (LessonDayEntryView) node).collect(Collectors.toList());
        layoutEntryViews(entryViews, dayView, contentX, contentY, contentWidth, contentHeight);
    }

    private void layoutSwimlane(LessonDayView dayView, double contentX, double contentY, double contentWidth, double contentHeight) {
        ...
        for (Calendar calendar : visibleCalendars) {
            ...
            List<LessonDayEntryView> entryViews = getChildren().stream().filter(node -> node instanceof LessonDayEntryView).map(node -> (LessonDayEntryView) node).filter(view -> {
                Calendar cal;
    ...
    private void layoutEntryViews(List<LessonDayEntryView> entryViews, LessonDayView dayView, double contentX, double contentY, double contentWidth, double contentHeight) {
    ...
    private boolean removeEntryView(Entry<?> entry) {
        boolean removed = getChildren().removeIf(node -> {
            if (node instanceof LessonDayEntryView) {
            	LessonDayEntryView view = (LessonDayEntryView) node;
    ...
    private LessonDayEntryView doAddEntryView(Entry<?> entry) {
        Callback<Entry<?>, LessonDayEntryView> factory = getSkinnable().getEntryViewFactory();
        LessonDayEntryView view = factory.call(entry);
    ...
    private void updateEntries(String reason) {
        ...
        getChildren().removeIf(node -> node instanceof LessonDayEntryView);
        ...
        LoggingDomain.VIEW.fine("updating entries in day view " + getSkinnable().getDate() + ": reason = " + reason + ", entry count: " + getChildren().stream().filter(child -> child instanceof LessonDayEntryView).count());
    }
