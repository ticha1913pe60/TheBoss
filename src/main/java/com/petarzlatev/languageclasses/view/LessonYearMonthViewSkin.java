...
public class LessonYearMonthViewSkin extends DateControlSkin<LessonYearMonthView> implements LoadDataSettingsProvider {
	...
	private Map<String, LessonYearMonthView.DateCell> cellsMap = new HashMap<>();
	...
	public LessonYearMonthViewSkin(LessonYearMonthView view) {
		super(view);
		...
		view.enableHyperlinksProperty().addListener(it -> updateHyerlinkSupport());
	...
	private void updateHyerlinkSupport() {
		final LessonYearMonthView view = getSkinnable();
	...
	protected void entryCalendarChanged(CalendarEvent evt) {
		...
		/*
		 * If an entry has been deleted (new calendar == null) then we have no way of
		 * finding out
		 */
		if (isRelevant(entry)) {
	...
	private void buildView() {
		...
		LessonYearMonthView view = getSkinnable();
		...
		if (getSkinnable().isShowMonth()) {
			if (getSkinnable().isShowMonthArrows()) {
				...
				prevMonthRegion
						.setOnMouseClicked(evt -> getSkinnable().setDate(getSkinnable().getDate().minusMonths(1)));
			}
			...
			if (getSkinnable().isShowMonthArrows()) {
				...
				nextMonthRegion
						.setOnMouseClicked(evt -> getSkinnable().setDate(getSkinnable().getDate().plusMonths(1)));
			}
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < 7; col++) {
				Callback<LessonYearMonthView, LessonYearMonthView.DateCell> cellFactory = view.getCellFactory();
				LessonYearMonthView.DateCell cell = cellFactory.call(getSkinnable());
		...
		if (getSkinnable().isShowTodayButton()) {
			Button button = new Button(Messages.getString("YearMonthViewSkin.TODAY")); //$NON-NLS-1$
	...
	private void updateView() {
		...
		monthLabel.setText(DateTimeFormatter.ofPattern(Messages.getString("YearMonthViewSkin.MONTH_FORMAT"), Locale.getDefault()) //$NON-NLS-1$
				.format(yearMonth));
		yearLabel.setText(DateTimeFormatter.ofPattern(Messages.getString("YearMonthViewSkin.YEAR_FORMAT"), Locale.getDefault()) //$NON-NLS-1$
				.format(yearMonth));
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < 7; col++) {
				...
				LessonYearMonthView.DateCell cell = cellsMap.get(getKey(row, col));
				cell.setDate(localDate);
				cell.getStyleClass().removeAll(TODAY, DAY_OF_MONTH_LABEL, DAY_NOT_OF_MONTH_LABEL, WEEKEND_DAY,
						SELECTED_MONTH_DATE);
	...
	private void handleSingleClick(MouseEvent evt, Node node, LocalDate date) {
		...
		case SHOW_DETAILS:
			...
			DateDetailsParameter param = new DateDetailsParameter(evt, view, node, date, evt.getScreenX(),
					evt.getScreenY());
			callback.call(param);
			break;
	...
	private void updateUsageColors(String reason) {
		cellsMap.values().forEach(control -> control.getStyleClass().removeAll(USAGE_VERY_LOW, USAGE_LOW, USAGE_MEDIUM,
				USAGE_HIGH, USAGE_VERY_HIGH));
		...
		LoggingDomain.VIEW
				.fine("updating colors: reason = " + reason + ", year month = " + getSkinnable().getYearMonth());

		Map<LocalDate, List<Entry<?>>> dataMap = new HashMap<>();
		dataLoader.loadEntries(dataMap);

		for (String key : cellsMap.keySet()) {
			LessonYearMonthView.DateCell cell = cellsMap.get(key);
	...
	public LocalDate getLoadStartDate() {
		/*
		 * The month view also shows the last couple of days of the previous month.
		 */
		return Util.adjustToFirstDayOfWeek(getSkinnable().getDate().withDayOfMonth(1),
