 ...
 */
public class LessonYearView extends DateControl {

    private static final String DEFAULT_STYLE_CLASS = "year-view";

    private Map<Month, LessonYearMonthView> viewMap = new HashMap<>();

    /**
     * Constructs a new year view.
     */
    public LessonYearView() {
        getStyleClass().add(DEFAULT_STYLE_CLASS);

        dateProperty().addListener(evt -> year.set(Year.from(getDate())));

        for (Month month : Month.values()) {
            LessonYearMonthView view = new LessonYearMonthView();
    ...
    protected Skin<?> createDefaultSkin() {
        return new LessonYearViewSkin(this);
    }
     ...
     */
    public final LessonYearMonthView getMonthView(Month month) {
