 ...
 */
public class LessonDayPage extends LessonPageBase {

	private LessonAgendaView agendaView;
	private LessonYearMonthView yearMonthView;
	private LessonDetailedDayView detailedDayView;
	 ...
	 */
	public LessonDayPage() {
		super();

		getStyleClass().add("day-page"); //$NON-NLS-1$

		setDateTimeFormatter(DateTimeFormatter.ofPattern(Messages.getString("DayPage.DATE_FORMATTER"), Locale.getDefault())); //$NON-NLS-1$

		this.agendaView = new LessonAgendaView();
		this.yearMonthView = new LessonYearMonthView();
		this.detailedDayView = new LessonDetailedDayView();
		this.toolBarControls = createToolBarControls();
	}

	@Override
	protected Skin<?> createDefaultSkin() {
		return new LessonDayPageSkin(this);
	}
	...
	private Node createToolBarControls() {
		...
		listIcon.getStyleClass().addAll("lesson-button-icon");
		...
		calendarIcon.getStyleClass().addAll("lesson-button-icon");
		...
		standardIcon.getStyleClass().addAll("lesson-button-icon");
		...
		layoutIcon.getStyleClass().addAll("lesson-button-icon", "layout-button-icon"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	/**
	 * Controls whether the "layout" button (to toggle between standard and swimlane
	 * layout) in the upper left corner of the control will be shown to the user or
	 * not.
	 *
	...
	/**
	 * An enumerator used for telling the {@link DayPage} view to show more or less
	 * content.
	 */
	public enum DayPageLayout {

		/**
		 * Specifies that the view will display a {@link DayView}, a {@link MonthView},
		 * and an {@link AgendaView}.
		 */
		STANDARD,
	...
	private final ObjectProperty<DayPageLayout> dayPageLayout = new SimpleObjectProperty<>(this, "dayPageLayout", //$NON-NLS-1$
			DayPageLayout.STANDARD);

	/**
	 * Stores the currently requested layout for the {@link DayPage}. The layout
	 * determines if the page will show only the {@link DayView} or a more complext
	 * UI with the additional {@link MonthView} for picking a date, and an
	 * {@link AgendaView} with the calendar entries for the next couple of weeks.
	 *
	...
	private final BooleanProperty showDayPageLayoutControls = new SimpleBooleanProperty(this,
			"showDayPageLayoutControls", true); //$NON-NLS-1$

	/**
	 * Determines if the controls for switching between different layouts of this
	 * view will be shown to the user or not.
	 *
	 ...
	 *
	 * @param show if true the controls will be shown
	 */
	 ...
	 */
	public final LessonAgendaView getAgendaView() {
		return agendaView;
	}
	 ...
	 */
	public final LessonYearMonthView getYearMonthView() {
	 ...
	 * @return the day view
	 */
	public final LessonDetailedDayView getDetailedDayView() {
