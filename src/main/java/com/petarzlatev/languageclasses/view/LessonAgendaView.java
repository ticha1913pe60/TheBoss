 ...
 * <p/>
 */
public class LessonAgendaView extends DateControl {
	...
	private final ListView<LessonAgendaEntry> listView = new ListView<>();
	private final ObjectProperty<DateTimeFormatter> formatter = new SimpleObjectProperty<>(this, "formatter", //$NON-NLS-1$
			DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG));

	/**
	 * Constructs a new agenda view.
	 */
	public LessonAgendaView() {
	 ...
	 */
	public final ListView<LessonAgendaEntry> getListView() {
		return listView;
	}

	@Override
	protected Skin<?> createDefaultSkin() {
		return new LessonAgendaViewSkin(this);
	}
	...
	/**
	 * Stores the number of days to "look ahead" into the future when loading its
	 * data.
	 *
	...
	private final ObjectProperty<Callback<LessonAgendaView, ? extends LessonAgendaEntryCell>> cellFactory = new SimpleObjectProperty<Callback<LessonAgendaView, ? extends LessonAgendaEntryCell>>(
			this, "cellFactory", view -> new LessonAgendaEntryCell(this)) {
		@Override
		public void set(Callback<LessonAgendaView, ? extends LessonAgendaEntryCell> newValue) {
			super.set(Objects.requireNonNull(newValue));
		}
	};

	public final ObjectProperty<Callback<LessonAgendaView, ? extends LessonAgendaEntryCell>> cellFactoryProperty() {
		return cellFactory;
	}

	public final Callback<LessonAgendaView, ? extends LessonAgendaEntryCell> getCellFactory() {
		return cellFactoryProperty().get();
	}

	public final void setCellFactory(Callback<LessonAgendaView, ? extends LessonAgendaEntryCell> cellFactory) {
		cellFactoryProperty().set(cellFactory);
	}

	/**
	 * Gets the DateTimeFormatter property, which is use to provide the format on
	 * the TimeScale Labels. By default it has a value of {@link FormatStyle#LONG}
	 * 
	 ...
	 */
	public static class LessonAgendaEntry implements Comparable<LessonAgendaEntry> {

		private LocalDate date;

		public LessonAgendaEntry(LocalDate date) {
		...
		@Override
		public int compareTo(LessonAgendaEntry o) {
			return getDate().compareTo(o.getDate());
		}
	}

	/**
	 * A specialized list cell that is capable of displaying all entries currently
	 * assigned to a given day. Each cell features a header that shows the date
	 * information and a body that lists all entries. Each entry is visualized with
	 * an icon, a title text, and a time text.
	 *
	 * @see AgendaView#getListView()
	 * @see ListView#setCellFactory(javafx.util.Callback)
	 */
	public static class LessonAgendaEntryCell extends ListCell<LessonAgendaEntry> {
		...
		private DateTimeFormatter weekdayFormatter = DateTimeFormatter
				.ofPattern(Messages.getString("AgendaEntryCell.WEEKDAY_FORMAT"), Locale.getDefault()); //$NON-NLS-1$
		...
		private final LessonAgendaView agendaView;
		 ...
		 */
		public LessonAgendaEntryCell(LessonAgendaView view) {
			this(view, true);
		}
		 ...
		 */
		public LessonAgendaEntryCell(LessonAgendaView view, boolean headerPaneVisible) {
		 ...
		 *
		 * In this default implementation the body consists of a grid pane with three
		 * columns. The middle column is used for showing the title of calendar entries.
		 * This column will get whatever space is left after the icon and the time
		 * column have used what they need. This means that a very long title will
		 ...
		 * with the weekday label in the "left" position and the date label in the
		 * "right" position.
		 *
		...
		@Override
		protected void updateItem(LessonAgendaEntry item, boolean empty) {
		...
		/**
		 * Creates the label used to display the time of the entry. The default
		 * implementation of this method creates a label and sets the text returned by
		 * {@link #getTimeText(Entry)}.
		 *
		...
		/**
		 * Creates the label used to display the title of the entry. The default
		 * implementation of this method creates a label and sets the text found in
		 * {@link Entry#getTitle()}.
		 *
		...
		/**
		 * Creates a node used to display an icon for the entry. The default
		 * implementation of this method creates a node of type {@link Circle}. The
		 * color of the circle will match the color of the calendar to which the entry
		 * belongs.
		 * 
		 * <pre>
		...
		/**
		 * Creates a nicely formatted text that contains the start and end time of the
		 * given entry. The text can also be something like "full day" if the entry is a
		 * full-day entry.
		 *
		...
		protected String getTimeText(Entry<?> entry) {
			...
			if (startDate.equals(endDate)) {
				...
				text = MessageFormat.format(Messages.getString("AgendaEntryCell.ENTRY_TIME_RANGE_WITH_DATE"), //$NON-NLS-1$
						shortDateFormatter.format(entry.getStartDate()), timeFormatter.format(entry.getStartTime()),
						shortDateFormatter.format(entry.getEndDate()), timeFormatter.format(entry.getEndTime()));
			}
		...
		/**
		 * Sets the Medium Date Formatter, the value by default is
		 * {@link FormatStyle#MEDIUM}. <br>
		...
		/**
		 * Sets the Short Date Formatter, the value by default is
		 * {@link FormatStyle#SHORT}. <br>
		...
		/**
		 * Sets the Time Formatter, the value by default is {@link FormatStyle#SHORT}.
		 * <br>
