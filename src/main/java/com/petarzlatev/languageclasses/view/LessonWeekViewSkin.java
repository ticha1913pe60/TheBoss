...
public class LessonWeekViewSkin extends SkinBase<LessonWeekView> {

    private GridPane dayGridPane = new GridPane();

    public LessonWeekViewSkin(LessonWeekView view) {
        super(view);
    ...
    private void buildDays() {
        LessonWeekView weekView = getSkinnable();
        List<LessonWeekDayView> weekDayViews = weekView.getWeekDayViews();
        ...
        Callback<LessonWeekView.WeekDayParameter, LessonWeekDayView> weekDayViewFactory = weekView.getWeekDayViewFactory();
        ...
        for (int i = 0; i < numberOfDays; i++) {
            ...
            LessonWeekView.WeekDayParameter param = new LessonWeekView.WeekDayParameter(weekView);
            LessonWeekDayView weekDayView = weekDayViewFactory.call(param);
...
//            weekDayView.endTimeProperty().bind(weekView.endTimeProperty());
    ...
    private void updateUsedTimes() {
        ...
        for (LessonWeekDayView view : getSkinnable().getWeekDayViews()) {
    ...
    private void updateDate(LessonWeekDayView view, int dayCount) {
        updateDate(view, getSkinnable().getDate(), dayCount);
    }

    private void updateDate(LessonWeekDayView view, LocalDate startDate, int dayCount) {
