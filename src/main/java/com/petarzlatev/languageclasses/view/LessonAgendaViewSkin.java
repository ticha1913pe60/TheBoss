...
import com.petarzlatev.languageclasses.view.LessonAgendaView.LessonAgendaEntry;
...
public class LessonAgendaViewSkin extends DateControlSkin<LessonAgendaView> implements LoadDataSettingsProvider {

	private static final String AGENDA_VIEW_PLACEHOLDER_LABEL = "placeholder-label"; //$NON-NLS-1$

	private ListView<LessonAgendaEntry> listView;

	private DataLoader dataLoader = new DataLoader(this);

	private Label statusLabel;

	public LessonAgendaViewSkin(LessonAgendaView view) {
		super(view);
	...
	private void updateList(String reason) {
		...
		List<LessonAgendaEntry> listEntries = new ArrayList<>();
		for (LocalDate date : dataMap.keySet()) {
			LessonAgendaEntry listViewEntry = new LessonAgendaEntry(date);
		...
		DateTimeFormatter formatter = (DateTimeFormatter.ofPattern(Messages.getString("AgendaView.DATE_FORMATTER"), //$NON-NLS-1$
				Locale.getDefault()));

		String startTime = formatter.format(getLoadStartDate());
		String endTime = formatter.format(getLoadEndDate());

		statusLabel.setText(MessageFormat.format(Messages.getString("AgendaViewSkin.AGENDA_TIME_RANGE"), //$NON-NLS-1$
				startTime, endTime));
	}
