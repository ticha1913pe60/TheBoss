...
public class LessonCalendarViewSkin extends SkinBase<LessonCalendarView> {
	...
	private LessonDayPage dayPage;
	private LessonWeekPage weekPage;
	private LessonMonthPage monthPage;
	private LessonYearPage yearPage;

	private List<LessonPageBase> pageList = new ArrayList<>();
	...
	public LessonCalendarViewSkin(LessonCalendarView view) {
		super(view);
		...
		addIcon.getStyleClass().addAll("lesson-button-icon", //$NON-NLS-1$
		...
		printIcon.getStyleClass().addAll("lesson-button-icon", "print-button-icon"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		final LessonPageBase selectedPage = view.getSelectedPage();
	...
	private void updateCalendarVisibility() {
		LessonCalendarView view = getSkinnable();
	...
	private void buildLeftToolBarBox() {
		...
		if (getSkinnable().isShowPageToolBarControls()) {
			LessonPageBase page = getSkinnable().getSelectedPage();
	...
	private void changePage() {
		...
		if (view.isTransitionsEnabled()) {
			...
			LessonPageBase selectedPage = view.getSelectedPage();
	...
	private void hideNonSelectedPages() {
		LessonPageBase selectedPage = getSkinnable().getSelectedPage();
	...
	private void updateToggleButtons() {
		LessonCalendarView view = getSkinnable();
		LessonPageBase page = view.getSelectedPage();
	...
	private void animateChangePage() {
		...
		LessonPageBase oldPage = null;

		final ObservableList<Node> children = stackPane.getChildren();
		if (!children.isEmpty()) {
			oldPage = (LessonPageBase) children.get(children.size() - 1);
		}
		...
		LessonPageBase newPage = getSkinnable().getSelectedPage();
