...
public class LessonDayViewEditController {
	...
	public LessonDayViewEditController(DayViewBase dayView) {
		...
		// mouse released is very important for us. register with the scene so we get
		// that in any case.
		...
		// also register with the scene property. Mostly to remove our event filter if
	...
	private void initDragModeAndHandle(MouseEvent evt) {
		...
		dayEntryView = (LessonDayEntryView) evt.getTarget();

		entry = (Entry<Lesson>) dayEntryView.getEntry();
		...
		if (y > dayEntryView.getHeight() - 5) {
			if (dayView.getEntryEditPolicy()
					.call(new DateControl.EntryEditParameter(dayView, entry, DateControl.EditOperation.CHANGE_END))) {
				dragMode = DraggedEntry.DragMode.END_TIME;
				handle = Handle.BOTTOM;
			}
		} else if (y < 5) {
			if (dayView.getEntryEditPolicy()
					.call(new DateControl.EntryEditParameter(dayView, entry, DateControl.EditOperation.CHANGE_START))) {
				dragMode = DraggedEntry.DragMode.START_TIME;
				handle = Handle.TOP;
			}
		} else {
			if (dayView.getEntryEditPolicy()
					.call(new DateControl.EntryEditParameter(dayView, entry, DateControl.EditOperation.MOVE))) {
	...
	private void mousePressed(MouseEvent evt) {
		...
		LessonDayView dayView = dayEntryView.getDateControl();
		if (dayView != null) {
	...
	private void mouseReleased(MouseEvent evt) {
		...
		/*
		 * We might run in the sampler application. Then the entry view will not be
		 * inside a date control.
		 */
		...
		if (draggedEntry != null) {
			...
			if (entry.getUserObject() != null) {
				if (!draggedEntry.getStartTime().equals(entry.getUserObject().getStart().toLocalTime())
						|| !draggedEntry.getEndTime().equals(entry.getUserObject().getEnd().toLocalTime())) {
					entry.getUserObject().setStart(draggedEntry.getStartDate().atTime(draggedEntry.getStartTime()));
					entry.getUserObject().setEnd(draggedEntry.getStartDate().atTime(draggedEntry.getEndTime()));
				}
				try {
					DataSource.getInstance().getLessonDAO().addLesson(entry);
	...
	private void mouseDragged(MouseEvent evt) {
		...
		if (dayView.getDraggedEntry() == null || dayEntryView.getParent() == null) {
			// we might see "mouse dragged" events close before "mouse pressed". in this
			// case, our drag/dro handling
	...
	private void changeStartTime(MouseEvent evt) {
		...
		if (isMinimumDuration(entry, entry.getEndAsLocalDateTime(), locationTime)) {
			...
			LOGGER.finer("new interval: sd = " + startDate + ", st = " + startTime + ", ed = " + endDate + ", et = "
					+ endTime);
	...
	private void requestLayout() {
		...
		if (dayView instanceof LessonWeekView) {
			((LessonWeekView) dayView).getWeekDayViews().forEach(Parent::requestLayout);
		}
	}
