...
public class OracleLessonDAO implements LessonDAO {
	...
	public List<Lesson> loadLessons() throws SQLException {
		...
		while (results.next()) {
			...
			Lesson lesson = new Lesson(student,
					...
					LocalDate.parse(results.getString(DataSource.COLUMN_LESSONS_DATE),DateTimeFormatter.ISO_LOCAL_DATE),
	...
	public int addLesson(Entry<Lesson> entry) throws SQLException {
		String returnCols[] = { DataSource.COLUMN_LESSONS_ID_LESSON };
		PreparedStatement insertLesson = conn.prepareStatement(INSERT_LESSON, returnCols);
